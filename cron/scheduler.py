    ...
    *,
    ...
    try:
        try:
            ...
            # Back-compat for tests/plugins that monkeypatch _build_job_prompt
            # with the pre-extra_prompt signature. Do not swallow TypeErrors
            # raised by the real builder body.
            if "extra_prompt" not in str(type_exc):
                raise
    ...
    try:
        ...
        try:
            ...
            # Fork: per-job fallback_providers (resolved earlier into
            # fallback_model) must survive upstream's global-only
            # get_fallback_chain reassignment below — an explicit empty
            # list disables fallbacks for this job and wins over globals.
            fb_list = fallback_model if fallback_model is not None else (get_fallback_chain(_cfg) or [])
            runtime = None
