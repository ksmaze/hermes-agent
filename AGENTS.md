...
## Fork-Specific Changes (KsMaze)

This checkout carries local fork behavior that must be preserved when merging
upstream release tags.

### Custom provider and model metadata

- `agent/models_dev.py` supports both upstream `model_overrides` and the fork's
  inline custom-provider metadata. Keep the merged API surface:
  `get_model_capabilities()` / `get_model_info()` accept upstream
  `allow_network=` plus fork `custom_providers=` and `base_url=`.
- Capability precedence is: models.dev catalog → upstream `model_overrides`
  (explicit entries patch catalog models; `_default` fills catalog misses) →
  fork inline `custom_providers[].models.<id>.context_length/capabilities`,
  which wins for fields it sets.
- Preserve fork-only `ModelCapabilities.supports_streaming`; inline
  `capabilities.streaming` sets it.
- Preserve custom-provider UX in `hermes_cli/model_switch.py`: inline
  `provider:model` routing, configured-provider exact matching, current-provider
  slug routing, custom provider grouping by endpoint/credential/api_mode/headers,
  and cached/live model discovery behavior.

### Cron jobs

- Cron jobs intentionally expose per-job `model`, `provider`, `base_url`, and
  `fallback_providers` pins through `tools/cronjob_tools.py`; this fork differs
  from upstream's narrower agent-facing schema.
- Preserve `normalize_job_fallback_providers()`, `_resolve_job_fallback_providers()`,
  per-job profile normalization, and the invariant that job-specific fallback
  lists win over global fallback config. An explicit empty job fallback list means
  “disable fallbacks for this job”.
- Preserve `disabled_skills` support via `get_effective_disabled_skill_names()`
  in cron/prompt-building paths.

### Tools and utilities

- Preserve `browser_scan` registration and scanner integration.
- Preserve skill-manager YOLO behavior: `is_current_session_yolo_enabled()` can
  bypass security scanning in the edit/create paths.
- Preserve `hermes_constants.json_dumps/json_dump` wrappers with
...
| Date | Upstream tag | Notes |
|------|--------------|-------|
| 2026-09-02 | v2026.8.31 | Merged upstream official `model_overrides` while preserving fork inline custom-provider capabilities, cron job model/fallback pins, browser_scan, skill-manager YOLO behavior, and JSON Unicode wrappers. |
