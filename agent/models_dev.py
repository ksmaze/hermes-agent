...
def get_model_capabilities(
    ...
    *,
    ...
    Resolution sources, lowest to highest precedence:
      1. models.dev catalog entry (or safe agentic defaults when the model
         is absent from the catalog).
      2. ``model_overrides`` config — EXPLICIT per-provider+model entries
         always win for the fields they set; ``_default`` entries fill the
         gap only for models the catalog does not know (#8731, #84482).
      3. Fork: per-model ``context_length`` / ``capabilities`` declared
         inline in ``custom_providers[].models.<id>`` (matched by
         ``custom:<slug>`` provider name or by ``base_url``).  These
         override both the catalog and ``model_overrides`` for the fields
         they set.
    ...
    # If no catalog entry, no model_overrides entry, and no inline custom
    # provider per-model config, we can't resolve capabilities.
    if (
        entry is None
        and override is None
        and not isinstance(custom_model_cfg, dict)
    ):
        return None
    ...
    # Apply model_overrides patches (each field is optional in the dict).
    ...
    # Fork: inline custom-provider per-model config (context_length +
    # capabilities) wins for the fields it sets — highest precedence.
...
def get_model_info(
    ...
    *,
    ...
    Fork: when ``custom_providers`` / ``base_url`` are supplied, per-model
    ``context_length`` / ``capabilities`` declared inline on
    ``custom_providers[].models.<id>`` are resolved through
    ``get_model_capabilities`` and win for the fields they set.
    ...
    # Apply model_overrides to whatever the catalog provided (explicit
    # entries patch known models; _default fills catalog misses).
    ...
    if raw_info is None:
        # Model not in catalog — an override (explicit or _default) may
        # still provide the metadata.
        raw_info = _from_override_alone()
    else:
        # Re-parse through the override merge so limit/modality sub-dicts
        # are patched, not clobbered.
        override = _override_for(provider_id, model_id, catalog_hit=True)
        if override is not None:
            raw_dict = pdata.get("models", {}).get(model_id) if isinstance(pdata, dict) else None
            if isinstance(raw_dict, dict):
                raw_info = _with_override(model_id, raw_dict)

    # Fork: overlay inline custom-provider per-model config. Only when a
    # custom_providers list (or base_url) is in play; the hot paths
    # (cost guard, inventory) never pass one and skip this entirely.
    if custom_providers is not None or base_url:
